-- Entry point for mpv-open-sub-lua.
--
...
local function do_search()
    ...
    if not video then
        mp.osd_message("mpv-open-sub-lua: no file loaded", 3)
        return
    end
    local provider = PROVIDERS[conf.provider]
    if not provider then
        mp.osd_message("mpv-open-sub-lua: unknown provider " .. tostring(conf.provider), 3)
        return
    end
...
mp.add_key_binding("n", "mpv-open-sub-lua", do_search)
mp.register_script_message("search", do_search)
msg.info("mpv-open-sub-lua loaded (press n, or script-message search)")
